...
static uint32_t rand_r_state = 0;
static struct ofputil_bucket *
group_stochastic_live_bucket(const struct xlate_ctx *ctx,
                       const struct group_dpif *group)
{
    struct ofputil_bucket *bucket = NULL;
    int32_t rand_num = 0, sum = 0;

    // initialize random seed once
    if (!is_srand_initialized) {
        rand_r_state = (uint32_t) time(NULL);
        is_srand_initialized = true;
    }

    LIST_FOR_EACH (bucket, list_node, &group->up.buckets) {
        if (bucket_is_alive(ctx, bucket, 0)) sum += bucket->weight;
    }
    if (sum == 0) return bucket;
    rand_num = rand_r(&rand_r_state);
    rand_num = rand_num > 0 ? rand_num % sum : (-rand_num) % sum;

    LIST_FOR_EACH (bucket, list_node, &group->up.buckets) {
        if (bucket_is_alive(ctx, bucket, 0)) {
            rand_num -= bucket->weight;
            if (rand_num < 0) return bucket;
        } else {
            xlate_report_bucket_not_live(ctx, bucket);
        }
    }
